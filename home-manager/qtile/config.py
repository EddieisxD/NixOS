...
keys = [
    ...
    Key(
        ...
        "p",
        lazy.spawn(
            """cmd=$(printf "Lock\nLogout\nReboot\nShutdown\nSuspend" | vicinae dmenu -n "Power") && case "$cmd" in Lock) loginctl lock-session ;; Logout) qtile cmd-obj -o cmd -f shutdown ;; Reboot) systemctl reboot ;; Shutdown) systemctl poweroff ;; Suspend) systemctl suspend ;; esac"""
        ),
        desc="powermenu",
    ),
    Key(
    ...
    Key(
        [], "XF86AudioMute", lazy.spawn("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle"), desc="Volume Mute"
    ),
    ...
    Key([mod], "h", lazy.spawn("vicinae toggle"), desc="clipboard"),  # placeholder — use xclip for real clipboard mgmt
...
def search():
    qtile.cmd_spawn("vicinae toggle")
...
bar_widgets = [
    ...
    widget.CurrentLayout(
        mode="icon",
        custom_icon_paths=[LAYOUT_ICONS],
        background="#353446",
        scale=0.50,
    ),
    widget.Image(
